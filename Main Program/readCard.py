...
def get_cards_with_string(jsonFile, name, offset):
    listOfCards = []
    currentNumber = 0
    for i in range(0, len(jsonFile)):
        if (name.upper() in jsonFile[i]["name"].upper()):
            currentNumber = currentNumber + 1
            if (currentNumber > offset):
                listOfCards.append(jsonFile[i]["name"] + "¼")
                if (currentNumber - offset > MAXCARDS):
                    break

    return str(listOfCards)
    ...
    with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as server_socket:
        ...
        while True:
            ...
            while Connected:
                ...
                if (message == "CLOSE"):
                    return_message == "Server Shutdown"
                    Connected = False
                else:
                    return_message = get_cards_with_string(jsonFile, message, 0)
